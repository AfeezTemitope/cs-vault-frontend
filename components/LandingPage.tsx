...
export default function LandingPage() {
  ...
  const steps = [
    { num: '01', icon: GraduationCap, role: 'Admin', title: 'Set Up The Platform', desc: 'Admin creates courses (CSC 497 and CSC 498), adds lecturers, and assigns them to their courses.', color: '#7C3AED' },
    { num: '02', icon: Users, role: 'Lecturer', title: 'Register Your Students', desc: 'Lecturers add students to their course. Each student automatically receives an email with their login credentials.', color: '#059669' },
    { num: '03', icon: FolderOpen, role: 'Student', title: 'Upload & Explore', desc: 'Students log in, submit their project, browse past work for reference, and receive grades and feedback.', color: '#D97706' },
  ];
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--ink)' }}>
      ...
      <nav style={{
        ...
        padding: '0 5%', height: 66, background: 'var(--card)',
        borderBottom: '1.5px solid var(--border)', boxShadow: 'var(--shadow-sm)',
      }}>
      ...
      <section style={{
        ...
        <div style={{ position: 'absolute', width: 500, height: 500, borderRadius: '50%', background: 'var(--accent-glow)', filter: 'blur(80px)', top: '10%', left: '50%', transform: 'translateX(-50%)', pointerEvents: 'none' }} />
        <div style={{ position: 'relative', zIndex: 1, maxWidth: 720 }}>
          <div className="fade-up" style={{ display: 'inline-flex', alignItems: 'center', gap: 6, background: 'var(--accent-light)', color: 'var(--accent)', padding: '6px 16px', borderRadius: 99, fontSize: 13, fontWeight: 600, marginBottom: 28, border: '1px solid var(--border)' }}>
            <Star size={12} fill="currentColor" /> Built for Computer Science Final Year Students
          </div>
          <h1 className="fade-up delay-1" style={{ fontSize: 'clamp(36px, 6vw, 68px)', fontWeight: 800, color: 'var(--soft)', letterSpacing: '-2px', lineHeight: 1.08, marginBottom: 22 }}>
            Your CS Projects,<br />
            <span style={{ background: 'linear-gradient(135deg, var(--accent) 0%, #C084FC 100%)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>
              All in One Place
            </span>
          </h1>
          <p className="fade-up delay-2" style={{ fontSize: 18, color: 'var(--muted)', maxWidth: 560, margin: '0 auto 40px', lineHeight: 1.75 }}>
          ...
          <div className="fade-up delay-3" style={{ display: 'flex', gap: 14, justifyContent: 'center', flexWrap: 'wrap', marginBottom: 40 }}>
          ...
          <div className="fade-up delay-4" style={{ display: 'flex', gap: 20, justifyContent: 'center', flexWrap: 'wrap', marginBottom: 48 }}>
          ...
          {/*<div className="fade-up delay-5">
            <Link href="/repository" style={{
              ...
              fontSize: 15, fontWeight: 600, border: '1.5px solid var(--border)',
              transition: 'all 0.2s',
            }}
      ...
      <section style={{ background: 'var(--surface)', padding: '72px 5%', borderTop: '1.5px solid var(--border)' }}>
        <div className="container" style={{ textAlign: 'center' }}>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, background: 'var(--accent-light)', color: 'var(--accent)', padding: '5px 14px', borderRadius: 99, fontSize: 12, fontWeight: 700, textTransform: 'uppercase' as const, letterSpacing: '0.5px', marginBottom: 16 }}>
            <Layers size={12} /> Supported Courses
          </div>
          <h2 style={{ fontSize: 'clamp(22px, 3.5vw, 32px)', fontWeight: 800, color: 'var(--soft)', letterSpacing: '-0.5px', marginBottom: 10 }}>Two courses, one platform</h2>
          <p style={{ color: 'var(--muted)', fontSize: 16, marginBottom: 40, maxWidth: 440, margin: '0 auto 40px' }}>CS-Vault is purpose-built for final year project management.</p>
      ...
      <section id="features" style={{ padding: '80px 5%' }}>
        <div className="container">
          <div style={{ textAlign: 'center', marginBottom: 56 }}>
            ...
            <h2 style={{ fontSize: 'clamp(22px, 3.5vw, 36px)', fontWeight: 800, color: 'var(--soft)', letterSpacing: '-0.5px', marginBottom: 10 }}>Everything your department needs</h2>
            <p style={{ color: 'var(--muted)', fontSize: 16, maxWidth: 480, margin: '0 auto' }}>A complete platform built for managing, submitting, and reviewing Computer Science final year projects.</p>
          </div>
      ...
      <section style={{ background: 'var(--surface)', padding: '80px 5%', borderTop: '1.5px solid var(--border)' }}>
        <div className="container">
          <div style={{ textAlign: 'center', marginBottom: 56 }}>
            ...
            <h2 style={{ fontSize: 'clamp(22px, 3.5vw, 36px)', fontWeight: 800, color: 'var(--soft)', letterSpacing: '-0.5px', marginBottom: 10 }}>Up and running in three steps</h2>
            <p style={{ color: 'var(--muted)', fontSize: 16, maxWidth: 440, margin: '0 auto' }}>Simple enough for any user, powerful enough for the whole department.</p>
          </div>
          <div className="grid-3">
            {steps.map((step, i) => (
              <div key={step.num} className={`card fade-up delay-${i + 1 as 1|2|3}`} style={{ padding: '32px 28px', position: 'relative', overflow: 'hidden' }}>
                <div style={{ position: 'absolute', top: 20, right: 20, fontFamily: 'JetBrains Mono', fontSize: 52, fontWeight: 900, color: 'var(--border)', lineHeight: 1, pointerEvents: 'none' }}>{step.num}</div>
      ...
      <section style={{ padding: '80px 5%' }}>
        <div className="container">
          <div style={{ background: 'linear-gradient(135deg, var(--accent) 0%, #9333EA 100%)', borderRadius: 24, padding: '56px 48px', textAlign: 'center', position: 'relative', overflow: 'hidden', boxShadow: '0 16px 48px var(--accent-glow)' }}>
            ...
            <h2 style={{ fontSize: 'clamp(22px, 3.5vw, 36px)', fontWeight: 800, color: '#fff', letterSpacing: '-0.5px', marginBottom: 14, position: 'relative' }}>Ready to get started?</h2>
            <p style={{ color: 'rgba(255,255,255,0.82)', fontSize: 16, marginBottom: 32, maxWidth: 420, margin: '0 auto 32px', position: 'relative', lineHeight: 1.7 }}>
              Contact your lecturer to get your login credentials, then sign in and start uploading.
            </p>
            <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap', position: 'relative' }}>
              <button onClick={() => router.push('/login')}
                style={{ background: '#fff', color: 'var(--accent)', border: 'none', borderRadius: 12, padding: '13px 28px', fontSize: 15, fontWeight: 700, cursor: 'pointer', display: 'inline-flex', alignItems: 'center', gap: 8 }}>
                Sign In <ArrowRight size={16} />
              </button>
              <Link href="/repository"
                style={{ background: 'rgba(255,255,255,0.15)', color: '#fff', border: '1.5px solid rgba(255,255,255,0.3)', borderRadius: 12, padding: '13px 28px', fontSize: 15, fontWeight: 700, cursor: 'pointer', display: 'inline-flex', alignItems: 'center', gap: 8, textDecoration: 'none' }}>
      ...
      <footer style={{ borderTop: '1.5px solid var(--border)', background: 'var(--card)', padding: '48px 5%' }}>
        <div className="container" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 24, textAlign: 'center' }}>
          ...
          <div>
            <p style={{ fontSize: 13, color: 'var(--muted)', marginBottom: 12, fontWeight: 600, textTransform: 'uppercase' as const, letterSpacing: '0.5px' }}>Developed by</p>
          ...
          <div style={{ display: 'flex', gap: 20 }}>
            <Link href="/login" style={{ color: 'var(--accent)', textDecoration: 'none', fontSize: 14, fontWeight: 600 }}>Sign In</Link>
            <Link href="/repository" style={{ color: 'var(--accent)', textDecoration: 'none', fontSize: 14, fontWeight: 600 }}>Repository</Link>
          </div>
